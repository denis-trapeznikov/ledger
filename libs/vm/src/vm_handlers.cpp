...
void VM::Handler__ForRangeIterate()
{
  ...
  if (instruction_->data == 2)
  {
    finished = ApplyFunctor<IntegerTypeIds>(variable.type_id,
	    [](auto &&variable, auto &&current, auto &&target) {
		    variable.Set(curr.Ref()++);
		    return variable.Get() >= target.Get();
	    }, variable, loop.current, loop.target);
  }
  else
  {
    finished = ApplyFunctor<IntegerTypeIds>(variable.type_id,
	    [](auto &&variable, auto &&current, auto &&target, auto &&delta) {
		    variable.Set(current.Get());
		    current.Set(current.Get() + delta.Get());
		    return variable.Get() >= target.Get();
	    }, variable, loop.current, loop.target, loop.delta);
  }
  if (finished)
  {
    pc_ = instruction_->index;
