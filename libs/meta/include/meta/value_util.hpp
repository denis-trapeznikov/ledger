...
/**
 * LeftAccumulate(f, a0, a1, a2, ..., an) returns f(f(...(f(a0, a1), a2), ...), an).
...
constexpr auto LeftAccumulate(F && /*unused*/, RV &&rv)
{
  return rv;
}

template <typename F, typename A, typename B, typename... Seq>
constexpr auto LeftAccumulate(F &&f, A &&a, B &&b, Seq &&... seq);

// The recursion base: last step, only two values left.
template <typename F, typename A, typename B>
constexpr auto LeftAccumulate(F &&f, A &&a, B &&b)
{
...
constexpr auto LeftAccumulate(F &&f, A &&a, B &&b, Seq &&... seq)
{
  return LeftAccumulate(std::forward<F>(f), f(std::forward<A>(a), std::forward<B>(b)),
                    std::forward<Seq>(seq)...);
}
